...
- Transaction-safe employment-history replacement
...
- Jobs restricted to approved companies with an administrator-verified email domain
...
- Company email badge only after an administrator verifies the official domain against the company proof
...
- Dedicated privacy policy and review-guidelines pages
- Keyboard focus indicators, skip navigation and reduced-motion support
...
If the previous package is already connected to a Supabase project, run `supabase/005_security_and_integrity_upgrade.sql`, followed by `supabase/004_verify_setup.sql`.

The upgrade clears previously self-declared company domains. An administrator must review each proof and set the verified domain again from the Admin dashboard.
...
10. Admin reviews the company proof and sets its verified email domain
11. Company email-domain badge appears after the authenticated email matches
12. Only that verified company account can submit a job for the company
13. Professional review with team-lead name and night-shift flag
14. Admin approves review
15. Advertisement submission and approval
16. Approved user/company previews another professional’s CV
17. Admin merges a deliberately created similar company
...
- Only an administrator may set or clear a company’s verified email domain.
- Advertisement destination links must use HTTP or HTTPS.
...
│   └── 005_security_and_integrity_upgrade.sql
├── Start_Local_Cloudflare.bat
