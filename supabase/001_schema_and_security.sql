...
begin
  ...
  if v_type='professional' then
    ...
    select id into v_company_id from public.companies
      where normalized_name = public.normalize_company_name(v_company_name)
      limit 1;
    v_domain := lower(nullif(trim(new.raw_user_meta_data->>'email_domain'),''));
    if v_domain is null then v_domain := public.email_domain(new.email); end if;
    if public.is_public_email_domain(v_domain) then v_domain := null; end if;
    ...
    if v_company_id is null then
      ...
      begin
        insert into public.companies(
          ...
          new.raw_user_meta_data->>'website',v_domain
        ) returning id into v_company_id;
      exception when unique_violation then
        select id into v_company_id from public.companies
        where normalized_name = public.normalize_company_name(v_company_name)
...
with check (poster_id=auth.uid() and public.is_approved_account() and is_real_confirmed=true);
...
grant update(location,worker_capacity_min,worker_capacity_max,compliance,provident_fund,salary_day_from,salary_day_to,overtime_paid,weekly_holiday,festival_bonus,night_shift,transport,canteen,website,email_domain,updated_at) on public.companies to authenticated;
grant execute on function public.find_company_matches(text) to anon,authenticated;
grant execute on function public.find_or_create_company(text,text) to authenticated;
