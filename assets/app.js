import { supabase, config, isConfigured, publicFileUrl } from './appwrite.js?v=2.0.16';
...
const T = {
  bn: {
    ...
    featuredCompanies:'ফিচারড কোম্পানি',latestProfessionals:'নতুন প্রফেশনাল',latestJobs:'সাম্প্রতিক চাকরি',noData:'এখনো কোনো তথ্য নেই।',employeeCount:'নিবন্ধিত কর্মী',reviews:'রিভিউ',location:'লোকেশন',teamLead:'টিম লিডার',nightShift:'নাইট শিফট',salary:'সেলারি',designation:'ডেজিগনেশন',description:'বিস্তারিত',postJob:'চাকরি পোস্ট করুন',deadline:'আবেদনের শেষ তারিখ',applyEmail:'আবেদনের ইমেইল',whatsapp:'হোয়াটসঅ্যাপ নম্বর (ঐচ্ছিক)',applyNow:'এখনই আবেদন করুন',applicationsClosed:'আবেদনের সময় শেষ',addCompany:'নতুন কোম্পানি যোগ করুন',submitAd:'বিজ্ঞাপন জমা দিন',editReview:'রিভিউ এডিট করুন',reviewUpdated:'রিভিউ আপডেট হয়েছে এবং পুনরায় অনুমোদনের জন্য পাঠানো হয়েছে।',
...
function activateTopCompanyRotation(companies){
  ...
  ratingRotationTimer=setInterval(()=>{
    ...
    const target=$('#topCompanyHighlightBody');if(!target)return;
    index=(index+1)%companies.length;target.innerHTML=topCompanyHighlightMarkup(companies[index],index+1,companies.length);
  },60000);
}
...
function topCompanyHighlightMarkup(company,position=1,total=1){
  const bn=state.lang==='bn';
  return `<div class="priority-icon company-icon">★</div><div class="priority-copy"><span class="eyebrow">${bn?'টপ রেটেড কোম্পানি':'Top-rated company'}</span><h3>${esc(company?.name||(bn?'কোম্পানি রেটিং দেখুন':'Explore company ratings'))}</h3><p>${company?`${number(company.rating).toFixed(1)} / 5 · ${company.reviewCount||0} ${esc(t('reviews'))}${total>1?` · ${position}/${total}`:''}`:(bn?'অনুমোদিত রিভিউ থেকে শীর্ষ কোম্পানি এখানে দেখা যাবে।':'The highest-rated company from approved reviews will appear here.')}</p></div><div class="priority-actions">${company?`<span class="priority-rating">${number(company.rating).toFixed(1)}</span><a class="button ghost small" href="#company?id=${encodeURIComponent(company.id)}">${esc(t('view'))} →</a>`:`<a class="button ghost small" href="#companies">${esc(t('companies'))} →</a>`}</div>`;
}
function heroPriorityMarkup(jobs,topCompany,topCompanyCount=1,liveJobCount=jobs.length){
  const bn=state.lang==='bn',job=jobs[0];
  return `<div class="hero-priority-grid"><article class="hero-priority-card priority-company" id="topCompanyHighlightBody">${topCompanyHighlightMarkup(topCompany,1,topCompanyCount)}</article><article class="hero-priority-card priority-job"><div class="priority-icon">JOB</div><div class="priority-copy"><span class="eyebrow">${bn?'হাইলাইটেড জব সার্কুলার':'Highlighted job circular'}</span><h3>${esc(job?.designation||(bn?'চলমান চাকরি দেখুন':'Explore current vacancies'))}</h3><p>${job?`${esc(job.company_name)} · ${esc(job.location)} · ${esc(t('deadline'))}: ${esc(jobDeadlineText(job))}`:(bn?'নতুন অনুমোদিত জব সার্কুলার এখানে সবার আগে দেখা যাবে।':'New approved job circulars will appear here first.')}</p></div><div class="priority-actions"><span class="priority-count">${liveJobCount}</span><a class="button dark small" href="#jobs">${bn?'সব চাকরি':'All jobs'} →</a></div></article></div>`;
}
function heroAdvertisementMarkup(ad){
  const bn=state.lang==='bn',image=ad?.banner_path?publicFileUrl(ad.banner_path):'';
  return `<article class="hero-advertisement-card"><div class="hero-ad-head"><div><span class="eyebrow">${bn?'প্রধান বিজ্ঞাপন':'Featured advertisement'}</span><h3>${esc(ad?.title||(bn?'আপনার বিজ্ঞাপন এখানে প্রদর্শিত হবে':'Your advertisement will appear here'))}</h3></div><span class="sponsored-pill">AD</span></div><div class="hero-ad-media">${image?`<img src="${esc(image)}" alt="${esc(ad.title)}">`:`<div class="hero-ad-placeholder"><strong>ADVERTISEMENT</strong><span>${bn?'ছবি ও ব্যানার প্রদর্শনের জায়গা':'Image and banner display area'}</span></div>`}</div><div class="hero-ad-details"><div><strong>${esc(ad?.advertiser_name||(bn?'PrepressHub বিজ্ঞাপন':'PrepressHub advertising'))}</strong><p>${esc(ad?.description||(bn?'কোম্পানি, পণ্য, সফটওয়্যার, ট্রেনিং, নিয়োগ ও ইন্ডাস্ট্রি সার্ভিসের বিজ্ঞাপন দিন।':'Promote companies, products, software, training, recruitment and industry services.'))}</p></div>${ad?.target_url?`<a class="button primary" href="${esc(ad.target_url)}" target="_blank" rel="noopener">${esc(t('view'))} →</a>`:`<a class="button primary" href="#advertise">${esc(t('submitAd'))} →</a>`}</div></article>`;
}
...
async function renderCompany(id){
  ...
  const [{data:employment},{data:reviews},{data:ownReviews}]=await Promise.all([
...
function reviewOverall(r){const ratings=['salary_benefits','work_environment','management','career_growth','work_life_balance'].map(key=>number(r[key]));let overall=ratings.reduce((sum,value)=>sum+value,0)/Math.max(1,ratings.length);if(r.night_shift)overall=Math.min(overall,2.5);return Number(overall.toFixed(2));}
function renderReview(r,editable=false){
  const reviewer=r.is_anonymous?t('anonymous'):(r.reviewer_name||'Professional');
  return `<article class="review"><div class="review-head"><div><strong>${esc(reviewer)}</strong>${r.status&&r.status!=='approved'?`<span class="review-status">${statusBadge(r.status)}</span>`:''}<p>${esc(r.employment_status==='current'?t('currentEmployee'):t('formerEmployee'))}</p></div><div><strong>${number(r.overall_rating||reviewOverall(r)).toFixed(1)}</strong><div class="stars">★★★★★</div></div></div><p><b>${esc(t('teamLead'))}:</b> ${esc(r.team_leader_name||'—')} (${number(r.team_lead_rating).toFixed(1)})</p><p><b>${esc(t('nightShift'))}:</b> ${r.night_shift?'Yes':'No'}</p><p><b>${esc(t('pros'))}:</b> ${nl2br(r.pros||'—')}</p><p><b>${esc(t('cons'))}:</b> ${nl2br(r.cons||'—')}</p>${editable?`<div class="review-actions"><button class="button ghost small" data-edit-review="${esc(r.id)}">${esc(t('editReview'))}</button></div>`:''}</article>`;
}
...
async function openAdminJobEditor(id){
  const [{data:job,error},{data:companies}]=await Promise.all([supabase.from('jobs').select('*').eq('id',id).maybeSingle(),supabase.from('companies').select('*').eq('status','approved').order('name',{ascending:true})]);
  if(error||!job)throw error||new Error('Job not found.');
  const options=(companies||[]).map(company=>`<option value="${esc(company.id)}" data-name="${esc(company.name)}" data-location="${esc(company.location||'')}" ${company.id===job.company_id?'selected':''}>${esc(company.name)}</option>`).join('');
  openModal(genericModal,`<div class="modal-body"><span class="eyebrow">${esc(t('adminDashboard'))}</span><h2>${esc(t('editJob'))}</h2><div class="notice warning">${state.lang==='bn'?'User বা Admin—যে-ই পোস্ট করুক, এখানে সব job information পরিবর্তন করা যাবে।':'All job information can be changed here regardless of whether a user or administrator created it.'}</div><form id="adminJobEditForm" data-job-id="${esc(job.id)}"><div class="form-grid"><label class="field"><span>${esc(t('companyName'))}</span><select name="companyId" id="adminJobCompanySelect" required>${options}</select></label><label class="field"><span>${esc(t('location'))}</span><input name="location" id="adminJobLocation" required value="${esc(job.location||'')}"></label><label class="field"><span>${esc(t('designation'))}</span><input name="designation" required value="${esc(job.designation||'')}"></label><label class="field"><span>${esc(t('salary'))}</span><input name="salary" required value="${esc(job.salary||'')}"></label><label class="field"><span>${esc(t('deadline'))}</span><input type="date" name="deadline" required value="${esc(job.deadline||'')}"></label><label class="field"><span>${esc(t('applyEmail'))}</span><input type="email" name="applyEmail" required value="${esc(job.apply_email||'')}"></label><label class="field"><span>${esc(t('whatsapp'))}</span><input type="tel" name="whatsapp" value="${esc(job.whatsapp_number||'')}" placeholder="8801XXXXXXXXX"><small>${state.lang==='bn'?'ঐচ্ছিক—খালি রাখা যাবে।':'Optional—may be left blank.'}</small></label><label class="field full"><span>${esc(t('description'))}</span><textarea name="description" required>${esc(job.description||'')}</textarea></label></div><div class="form-actions"><button class="button primary" type="submit">${esc(t('save'))}</button><button class="button ghost" type="button" data-close-dialog>${esc(t('cancel'))}</button></div></form></div>`);
  $('#adminJobCompanySelect')?.addEventListener('change',event=>{$('#adminJobLocation').value=event.target.selectedOptions[0]?.dataset.location||'';});
}
async function saveAdminJob(form){
  const fd=new FormData(form),selected=form.elements.companyId.selectedOptions[0],whatsapp=fd.get('whatsapp').trim();
  if(whatsapp&&whatsapp.replace(/\D/g,'').length<10)throw new Error('Enter a valid WhatsApp number with country code or leave it blank.');
  const changes={company_id:fd.get('companyId'),company_name:selected?.dataset.name||selected?.textContent||'',location:fd.get('location').trim(),designation:fd.get('designation').trim(),salary:fd.get('salary').trim(),deadline:fd.get('deadline'),apply_email:fd.get('applyEmail').trim().toLowerCase(),whatsapp_number:whatsapp,description:fd.get('description').trim()};
  const {error}=await supabase.from('jobs').update(changes).eq('id',form.dataset.jobId);if(error)throw error;
  closeModal(genericModal);showToast(t('success'));await refreshLiveJobBadge();await renderAdmin();
}
async function openAdminAdEditor(id){
  const {data:ad,error}=await supabase.from('advertisements').select('*').eq('id',id).maybeSingle();if(error||!ad)throw error||new Error('Advertisement not found.');
  const image=ad.banner_path?publicFileUrl(ad.banner_path):'';
  openModal(genericModal,`<div class="modal-body"><span class="eyebrow">${esc(t('adminDashboard'))}</span><h2>${esc(t('editAdvertisement'))}</h2>${image?`<div class="ad-media"><img src="${esc(image)}" alt="${esc(ad.title||'Advertisement')}"></div>`:''}<form id="adminAdEditForm" data-ad-id="${esc(ad.id)}" data-ad-status="${esc(ad.status||'pending')}"><div class="form-grid"><label class="field"><span>${esc(t('adTitle'))}</span><input name="title" required value="${esc(ad.title||'')}"></label><label class="field"><span>${esc(t('advertiser'))}</span><input name="advertiser" required value="${esc(ad.advertiser_name||'')}"></label><label class="field"><span>${esc(t('email'))}</span><input type="email" name="email" required value="${esc(ad.contact_email||'')}"></label><label class="field"><span>${esc(t('phone'))}</span><input name="phone" value="${esc(ad.phone||'')}"></label><label class="field"><span>${esc(t('placement'))}</span><select name="placement"><option value="homepage" ${ad.placement==='homepage'?'selected':''}>${esc(t('homepage'))}</option><option value="directory" ${ad.placement==='directory'?'selected':''}>${esc(t('directory'))}</option><option value="company" ${ad.placement==='company'?'selected':''}>${esc(t('companyPage'))}</option></select></label><label class="field"><span>${esc(t('targetUrl'))}</span><input type="url" name="targetUrl" value="${esc(ad.target_url||'')}"></label><label class="field full"><span>${esc(t('description'))}</span><textarea name="description" required>${esc(ad.description||'')}</textarea></label><label class="field full"><span>${esc(t('banner'))}</span><input type="file" name="banner" accept="image/png,image/jpeg,image/webp"><small>${state.lang==='bn'?'নতুন ছবি না দিলে বর্তমান banner অপরিবর্তিত থাকবে।':'Leave empty to keep the current banner.'}</small></label></div><div class="form-actions"><button class="button primary" type="submit">${esc(t('save'))}</button><button class="button ghost" type="button" data-close-dialog>${esc(t('cancel'))}</button></div></form></div>`);
}
async function saveAdminAd(form){
  const fd=new FormData(form),target=fd.get('targetUrl').trim(),file=form.elements.banner.files[0];
  if(target){const parsed=new URL(target);if(!['http:','https:'].includes(parsed.protocol))throw new Error('Only HTTP or HTTPS destination links are allowed.');}
  const changes={title:fd.get('title').trim(),advertiser_name:fd.get('advertiser').trim(),contact_email:fd.get('email').trim().toLowerCase(),phone:fd.get('phone').trim(),placement:fd.get('placement'),description:fd.get('description').trim(),target_url:target||null};
  if(file){const ext=file.name.split('.').pop().toLowerCase(),path=`${state.user.id}/admin-ad-${crypto.randomUUID()}.${ext}`;const {error:uploadError}=await supabase.storage.from('ad-banners').upload(path,file,{upsert:false});if(uploadError)throw uploadError;changes.banner_path=path;}
  const {error}=await supabase.from('advertisements').update(changes).eq('id',form.dataset.adId);if(error)throw error;
  if(form.dataset.adStatus==='approved')await rpc('admin_moderate_content',{p_kind:'advertisements',p_id:form.dataset.adId,p_status:'approved'});
  closeModal(genericModal);showToast(t('success'));await renderAdmin();
}
...
function adminTableContent(kind,rows){
  ...
  return `<section class="admin-section"><h2>${esc(title)}</h2><table class="data-table"><thead><tr><th>Item</th><th>${esc(t('status'))}</th><th>${esc(t('created'))}</th><th>${esc(t('actions'))}</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${esc(kind==='reviews'?(r.companies?.name||'Review'):kind==='jobs'?r.designation:r.title)}</strong><small>${esc(kind==='reviews'?(r.is_guest?`${r.guest_name||'Guest'} · ${r.guest_email||''}`:(r.profiles?.full_name||'Anonymous')):kind==='jobs'?`${r.company_name} · ${r.deadline||'No deadline'}`:r.advertiser_name)}</small></td><td>${statusBadge(r.status)}</td><td>${formatDate(r.created_at)}</td><td><div class="table-actions">${kind==='reviews'?`<button class="neutral" data-edit-review="${esc(r.id)}">${esc(t('editReview'))}</button>`:kind==='jobs'?`<button class="neutral" data-admin-edit-job="${esc(r.id)}">${esc(t('editJob'))}</button>`:`<button class="neutral" data-admin-edit-ad="${esc(r.id)}">${esc(t('editAdvertisement'))}</button>`}<button class="approve" data-admin-content="${kind}" data-status="approved" data-id="${esc(r.id)}">${esc(t('approve'))}</button><button class="reject" data-admin-content="${kind}" data-status="rejected" data-id="${esc(r.id)}">${esc(t('reject'))}</button><button class="delete" data-admin-content="${kind}" data-status="deleted" data-id="${esc(r.id)}">${esc(t('delete'))}</button></div></td></tr>`).join('')}</tbody></table></section>`;
}
function adminTableGuestLogins(rows){
  return `<section class="admin-section"><h2>${esc(t('guestHistory'))}</h2><p class="muted">${state.lang==='bn'?'প্রতিবার guest access নেওয়ার সময় নাম, email, সময় ও browser/device record হয়। এই তথ্য শুধু Admin দেখতে পারবেন।':'Each guest access records the name, email, time and browser/device. Only administrators can view this information.'}</p><table class="data-table"><thead><tr><th>${esc(t('fullName'))}</th><th>${esc(t('email'))}</th><th>${esc(t('created'))}</th><th>Browser / device</th></tr></thead><tbody>${rows.length?rows.map(row=>`<tr><td><strong>${esc(row.guest_name||'Guest')}</strong><small>${esc(row.id)}</small></td><td>${esc(row.guest_email||'—')}</td><td>${formatDate(row.logged_in_at||row.created_at)}</td><td><small>${esc(row.user_agent||'—')}</small></td></tr>`).join(''):`<tr><td colspan="4">${esc(t('noData'))}</td></tr>`}</tbody></table></section>`;
}
...
async function saveGuestSession(form){
  const fd=new FormData(form),guest={name:fd.get('name').trim(),email:fd.get('email').trim().toLowerCase()};
  const {error}=await supabase.guestLogin({guest});if(error)throw error;
...
async function submitJob(form){
  ...
  if(whatsapp&&String(whatsapp).replace(/\D/g,'').length<10)throw new Error('Enter a valid WhatsApp number with country code or leave it blank.');
...
function reviewRatingOptions(value=1){return [1,2,3,4,5].map(n=>`<option value="${n}" ${number(value)===n?'selected':''}>${n}</option>`).join('');}
function openReviewForm(companyId,review=null){
  const editing=Boolean(review?.id);
  if(editing&&!state.user)return showToast(t('loginRequired'),'error');
  if(!editing&&!state.user&&!state.guest){state.pendingGuestReviewCompanyId=companyId;openGuestLogin();return;}
  ...
  const editNotice=editing?`<div class="notice warning">${state.profile?.role==='admin'?(state.lang==='bn'?'Admin হিসেবে রিভিউটি পরিবর্তন করছেন।':'You are editing this review as administrator.'):(state.lang==='bn'?'পরিবর্তনের পর রিভিউটি আবার Admin approval-এর জন্য যাবে।':'After editing, the review will return to administrator approval.')}</div>`:'';
  openModal(genericModal,`<div class="modal-body"><span class="eyebrow">Company review</span><h2>${esc(editing?t('editReview'):t('writeReview'))}</h2>${guestNotice}${editNotice}<form id="reviewForm" data-company-id="${esc(companyId)}" ${editing?`data-review-id="${esc(review.id)}"`:''}><div class="form-grid"><label class="field"><span>Employment</span><select name="employment"><option value="current" ${review?.employment_status!=='former'?'selected':''}>${esc(t('currentEmployee'))}</option><option value="former" ${review?.employment_status==='former'?'selected':''}>${esc(t('formerEmployee'))}</option></select></label><label class="field"><span>${esc(t('teamLead'))}</span><input name="teamLeadName" required maxlength="120" value="${esc(review?.team_leader_name||'')}"></label>${[['salaryBenefits','salary_benefits'],['workEnvironment','work_environment'],['management','management'],['careerGrowth','career_growth'],['workLife','work_life_balance'],['teamLeadRating','team_lead_rating']].map(([label,name])=>`<label class="field"><span>${esc(t(label))}</span><select name="${name}" required>${reviewRatingOptions(review?.[name]||1)}</select></label>`).join('')}<label class="check"><input type="checkbox" name="nightShift" ${review?.night_shift?'checked':''}><span>${esc(t('nightShift'))}</span></label><label class="check"><input type="checkbox" name="anonymous" ${review?.is_anonymous||(!editing&&state.guest&&!state.user)?'checked':''}><span>${esc(t('anonymous'))}</span></label><label class="field full"><span>${esc(t('pros'))}</span><textarea name="pros" required maxlength="2000">${esc(review?.pros||'')}</textarea></label><label class="field full"><span>${esc(t('cons'))}</span><textarea name="cons" required maxlength="2000">${esc(review?.cons||'')}</textarea></label><label class="field full"><span>${esc(t('advice'))}</span><textarea name="advice" maxlength="2000">${esc(review?.advice||'')}</textarea></label></div><div class="form-actions"><button class="button primary full" type="submit">${esc(editing?t('save'):t('submitReview'))}</button></div></form></div>`);
}
async function openReviewEditor(id){
  if(!state.user)return showToast(t('loginRequired'),'error');
  const {data:review,error}=await supabase.from('reviews').select('*').eq('id',id).maybeSingle();if(error||!review)throw error||new Error('Review not found.');
  const allowed=state.profile?.role==='admin'||review.reviewer_id===state.user.id;if(!allowed)throw new Error('You can edit only your own review.');
  openReviewForm(review.company_id,review);
}
async function submitReview(form){
  ...
  if(form.dataset.reviewId){const {company_id,status,...changes}=review;const {error}=await supabase.from('reviews').update(changes).eq('id',form.dataset.reviewId);if(error)throw error;}
  else if(state.user){const {error}=await supabase.from('reviews').insert({...review,reviewer_id:state.user.id});if(error)throw error;}
  ...
  showToast(form.dataset.reviewId?t('reviewUpdated'):t('reviewPending'));closeModal(genericModal);renderCompany(form.dataset.companyId);
}
...
async function handleClick(event){
  ...
  if(target.matches('[data-edit-review]')){await openReviewEditor(target.dataset.editReview);return;}
  ...
  if(target.matches('[data-admin-edit-job]')){await openAdminJobEditor(target.dataset.adminEditJob);return;}
  if(target.matches('[data-admin-edit-ad]')){await openAdminAdEditor(target.dataset.adminEditAd);return;}
...
async function handleSubmit(event){
  ...
  try{
    ...
    if(form.id==='guestLoginForm'){await saveGuestSession(form);return;}
    ...
    if(form.id==='adminJobEditForm'){await saveAdminJob(form);return;}
    if(form.id==='adminAdEditForm'){await saveAdminAd(form);return;}
