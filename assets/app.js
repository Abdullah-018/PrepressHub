import { supabase, config, isConfigured, publicFileUrl } from './appwrite.js?v=2.0.14';
...
function heroPriorityMarkup(jobs,ads,liveJobCount=jobs.length){
  const bn=state.lang==='bn',job=jobs[0],ad=ads[0],adImage=ad?.banner_path?publicFileUrl(ad.banner_path):'';
  return `<div class="hero-priority-grid"><article class="hero-priority-card priority-job"><div class="priority-icon">JOB</div><div class="priority-copy"><span class="eyebrow">${bn?'জরুরি জব সার্কুলার':'Priority job circular'}</span><h3>${esc(job?.designation||(bn?'চলমান চাকরি দেখুন':'Explore current vacancies'))}</h3><p>${job?`${esc(job.company_name)} · ${esc(job.location)} · ${esc(t('deadline'))}: ${esc(jobDeadlineText(job))}`:(bn?'নতুন অনুমোদিত জব সার্কুলার এখানে সবার আগে দেখা যাবে।':'New approved job circulars will appear here first.')}</p></div><div class="priority-actions"><span class="priority-count">${liveJobCount}</span><a class="button dark small" href="#jobs">${bn?'সব চাকরি':'All jobs'} →</a></div></article><article class="hero-priority-card priority-ad">${adImage?`<div class="priority-ad-image"><img src="${esc(adImage)}" alt="${esc(ad.title)}"></div>`:`<div class="priority-icon ad-icon">AD</div>`}<div class="priority-copy"><span class="eyebrow">${bn?'হাইলাইটেড বিজ্ঞাপন':'Featured advertisement'}</span><h3>${esc(ad?.title||(bn?'আপনার বিজ্ঞাপন এখানে দিন':'Promote your business here'))}</h3><p>${esc(ad?.advertiser_name||(bn?'ইন্ডাস্ট্রি সার্ভিস, ট্রেনিং ও নিয়োগ বিজ্ঞাপন':'Industry services, training and recruitment'))}</p></div><div class="priority-actions">${ad?.target_url?`<a class="button primary small" href="${esc(ad.target_url)}" target="_blank" rel="noopener">${esc(t('view'))} →</a>`:`<a class="button primary small" href="#advertise">${esc(t('advertise'))} →</a>`}</div></article></div>`;
}
