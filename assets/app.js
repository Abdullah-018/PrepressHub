...
function setLanguage(lang){
  state.lang=lang;localStorage.setItem('prepresshub_lang',lang);document.body.dataset.lang=lang;document.documentElement.lang=lang;$('#languageButton').textContent=lang==='bn'?'EN':'বাংলা';applyStaticTranslations();renderRoute();
}
...
async function signupCompany(form){
  ...
  const meta={account_type:'company',full_name:fd.get('contactPerson').trim(),phone:fd.get('phone').trim(),location:fd.get('location').trim(),company_name:fd.get('companyName').trim(),company_location:fd.get('location').trim(),worker_capacity_min:Number(fd.get('capacityMin')),worker_capacity_max:Number(fd.get('capacityMax')),compliance:fd.get('compliance')==='on',provident_fund:fd.get('providentFund')==='on',salary_day_from:Number(fd.get('salaryFrom')),salary_day_to:Number(fd.get('salaryTo')),overtime_paid:fd.get('overtimePaid')==='on',weekly_holiday:fd.get('weeklyHoliday')==='on',festival_bonus:fd.get('festivalBonus')==='on',night_shift:fd.get('nightShift')==='on',transport:fd.get('transport')==='on',canteen:fd.get('canteen')==='on',website:fd.get('website').trim(),email_domain:fd.get('emailDomain').trim()};
  const {data,error}=await supabase.auth.signUp({email,password:fd.get('password'),options:{emailRedirectTo:`${location.origin}${location.pathname}?next=account`,data:meta}});if(error)throw error;
...
async function saveProfile(form){
  ...
  const companyId=await rpc('find_or_create_company',{p_name:fd.get('currentCompany').trim(),p_location:fd.get('location').trim()});
  const {error:deleteError}=await supabase.from('employment_history').delete().eq('user_id',state.user.id);if(deleteError)throw deleteError;
  const rows=[{user_id:state.user.id,company_id:companyId,company_name_snapshot:fd.get('currentCompany').trim(),designation:fd.get('currentDesignation').trim(),is_current:true}];
  $$('.experience-row').forEach(row=>{const company=row.querySelector('[data-exp="company"]').value.trim(),designation=row.querySelector('[data-exp="designation"]').value.trim();if(company&&designation)rows.push({user_id:state.user.id,company_name_snapshot:company,designation,start_date:row.querySelector('[data-exp="from"]').value||null,end_date:row.querySelector('[data-exp="to"]').value||null,is_current:false});});
  const {error:insertError}=await supabase.from('employment_history').insert(rows);if(insertError)throw insertError;
...
async function saveCompanyProfile(form){
  const fd=new FormData(form);const update={location:fd.get('location').trim(),worker_capacity_min:Number(fd.get('capacityMin')||0),worker_capacity_max:Number(fd.get('capacityMax')||0),salary_day_from:Number(fd.get('salaryFrom')||0)||null,salary_day_to:Number(fd.get('salaryTo')||0)||null,website:fd.get('website').trim(),email_domain:fd.get('emailDomain').trim().toLowerCase(),compliance:fd.get('compliance')==='on',provident_fund:fd.get('provident_fund')==='on',overtime_paid:fd.get('overtime_paid')==='on',weekly_holiday:fd.get('weekly_holiday')==='on',festival_bonus:fd.get('festival_bonus')==='on',night_shift:fd.get('night_shift')==='on',transport:fd.get('transport')==='on',canteen:fd.get('canteen')==='on',updated_at:new Date().toISOString()};const {error}=await supabase.from('companies').update(update).eq('id',state.company.id);if(error)throw error;await rpc('sync_company_email_badge',{p_user_id:state.user.id});await refreshAuth();showToast(t('companySaved'));renderAccount();
}
...
function openJobForm(){
  if(!requireApproved())return;openModal(genericModal,`<div class="modal-body"><span class="eyebrow">Job circular</span><h2>${esc(t('postJob'))}</h2><div class="notice warning">${esc(t('realJobWarning'))}</div><form id="jobForm"><div class="form-grid"><label class="field"><span>${esc(t('companyName'))}</span><input name="companyName" required value="${esc(state.company?.name||'')}"></label><label class="field"><span>${esc(t('location'))}</span><input name="location" required></label><label class="field"><span>${esc(t('salary'))}</span><input name="salary" required></label><label class="field"><span>${esc(t('designation'))}</span><input name="designation" required></label><label class="field full"><span>${esc(t('description'))}</span><textarea name="description" required></textarea></label></div><label class="check"><input type="checkbox" name="truth" required><span>${esc(t('truthConfirm'))}</span></label><div class="form-actions"><button class="button primary full" type="submit">${esc(t('submit'))}</button></div></form></div>`);
}
async function submitJob(form){const fd=new FormData(form);const {error}=await supabase.from('jobs').insert({poster_id:state.user.id,company_id:state.profile?.company_id||null,company_name:fd.get('companyName').trim(),location:fd.get('location').trim(),salary:fd.get('salary').trim(),designation:fd.get('designation').trim(),description:fd.get('description').trim(),is_real_confirmed:true,status:'pending'});if(error)throw error;showToast(t('jobPending'));closeModal(genericModal);renderJobs();}
function openAdForm(){
...
async function submitAd(form){const fd=new FormData(form),file=form.elements.banner.files[0],ext=file.name.split('.').pop().toLowerCase(),path=`${state.user.id}/ad-${crypto.randomUUID()}.${ext}`;const {error:uploadError}=await supabase.storage.from('ad-banners').upload(path,file,{upsert:false});if(uploadError)throw uploadError;const {error}=await supabase.from('advertisements').insert({poster_id:state.user.id,title:fd.get('title').trim(),advertiser_name:fd.get('advertiser').trim(),contact_email:fd.get('email').trim(),phone:fd.get('phone').trim(),placement:fd.get('placement'),description:fd.get('description').trim(),target_url:fd.get('targetUrl').trim()||null,banner_path:path,status:'pending'});if(error)throw error;showToast(t('adPending'));closeModal(genericModal);renderAdvertisements();}
...
async function boot(){
  ...
  window.addEventListener('hashchange',()=>{ $('#mobileNav').classList.remove('open');renderRoute(); });
  if(isConfigured){
