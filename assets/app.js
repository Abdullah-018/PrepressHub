import { supabase, config, isConfigured, publicFileUrl } from './appwrite.js?v=2.0.12';
...
async function openJobForm(){
  ...
  openModal(genericModal,`<div class="modal-body"><span class="eyebrow">Job circular</span><h2>${esc(t('postJob'))}</h2><div class="notice warning">${state.lang==='bn'?'চাকরিটি Admin অনুমোদনের পর প্রকাশিত হবে।':'The job will be published after administrator approval.'}</div><form id="jobForm"><div class="form-grid"><label class="field"><span>${esc(t('companyName'))}</span><select name="companyId" id="jobCompanySelect" required>${options}</select></label><label class="field"><span>${esc(t('location'))}</span><input id="jobLocationInput" name="location" required value="${esc(companies[0].location||'')}"></label><label class="field"><span>${esc(t('salary'))}</span><input name="salary" required></label><label class="field"><span>${esc(t('designation'))}</span><input name="designation" required></label><label class="field full"><span>${esc(t('description'))}</span><textarea name="description" required></textarea></label></div><label class="check"><input type="checkbox" name="truth" required><span>${esc(t('truthConfirm'))}</span></label><div class="form-actions"><button class="button primary full" type="submit">${esc(t('submit'))}</button></div></form></div>`);
