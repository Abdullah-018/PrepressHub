import { supabase, config, isConfigured, publicFileUrl } from './appwrite.js?v=2.0.9';
...
function renderProfessionalCard(p){
  ...
  return `<article class="card${p.role==='admin'?' admin-profile-card':''}"><div class="card-head">${avatarMarkup(p,p.full_name)}<div><div>${adminBadge(p)} ${companyEmailBadge(p)}</div><h3>${esc(p.full_name||'Professional')}</h3><p>${esc(current?.designation||'—')}</p></div></div><div class="fact"><small>${esc(t('currentCompany'))}</small><b>${esc(current?.companies?.name||current?.company_name_snapshot||'—')}</b></div><div class="skill-cloud">${(p.skills||[]).slice(0,5).map(s=>`<span>${esc(s)}</span>`).join('')}</div><div class="card-foot"><span>${esc(current?.location||p.location||'Bangladesh')}</span><a href="#professional?id=${encodeURIComponent(p.id)}">${esc(t('view'))} →</a></div></article>`;
}
